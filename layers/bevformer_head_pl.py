...
from mmdet.core import build_assigner, build_sampler #reduce_mean, multi_apply  # ✅ 정리
#from mmdet.models.utils.transformer import inverse_sigmoid
# from mmdet.models import HEADS   # ❌ 불필요
# from mmdet.models.dense_heads import DETRHead  # ❌ 비상속이므로 불필요
...
##--------- mmcv 의존성 최소화-----------------------------
from .bbox_coder import NMSFreeCoder, normalize_bbox
from .utils import inverse_sigmoid, multi_apply
from .transformer import LearnedPositionalEncoding

#@HEADS.register_module()
class BEVFormerHeadPL(nn.Module):  # nn.Module 상속
    def __init__(self,
                 num_classes,
                 in_channels,
                 num_query=900,
        ...
        # --- cls avg factor 파라미터 ---
        self.sync_cls_avg_factor = kwargs.get('sync_cls_avg_factor', False)  # ✅
        self.bg_cls_weight = 0.0
        if isinstance(loss_cls, dict):
            self.bg_cls_weight = loss_cls.get('bg_cls_weight', 0.0)  # ✅

        # --- bbox coder ---
        bbox_coder.pop('type', None)
        self.bbox_coder = NMSFreeCoder(**bbox_coder) #build_bbox_coder(bbox_coder)
        ...
        self.code_weights = nn.Parameter(torch.tensor(code_weights, dtype=torch.float32),
                                         requires_grad=False)

        # --- transformer ---
        assert transformer is not None, 'transformer config must be provided'  # ✅
        ...
        self.positional_encoding = LearnedPositionalEncoding(**pos_enc_cfg)# build_positional_encoding(pos_enc_cfg)

        # --- 손실 함수 ---
        loss_cls.pop('type', None)  # 'type' 키 제거
        self.loss_cls = FocalLoss(**loss_cls) #build_loss(loss_cls)
        loss_bbox.pop('type', None)  # 'type' 키 제거
        self.loss_bbox = L1Loss(**loss_bbox) #build_loss(loss_bbox)
        loss_iou.pop('type', None) # 'type' 키 제거
        self.loss_iou = GIoULoss(**loss_iou) #build_loss(loss_iou)
        ...
        # ❌ 여기서 query/bev embedding 만들지 말 것 (아래 _init_layers에서 처리)
        self._init_layers()    
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        #gpt가 수정하라는데? 수정!!!!
        bev_mask = torch.zeros((bs, self.bev_h, self.bev_w), device=bev_queries.device, dtype=torch.bool) 
        bev_pos = self.positional_encoding(bev_mask) # pos는 float로 나옵니다
    ...
    def loss_single(self,
        ...
        if self.sync_cls_avg_factor:
            cls_avg_factor = cls_scores.new_tensor([cls_avg_factor]) #reduce_mean()
        ...
        num_total_pos = torch.clamp(num_total_pos, min=1).item()#torch.clamp(reduce_mean(num_total_pos), min=1).item()
...
#@HEADS.register_module()
class BEVFormerHead_GroupDETR(BEVFormerHeadPL):
    def __init__(self,
                 *args,
                 group_detr=1,
                 **kwargs):
